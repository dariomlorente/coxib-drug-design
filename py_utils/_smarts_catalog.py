"""
SMARTS catalogs for Brenk and PAINS structural alert filters.

This module contains the structural alert patterns used by filter_BrenkPAINS.
The Brenk alerts are taken from the full catalog (105 patterns), and PAINS
patterns are organized by class (a, b, c) with 480 total patterns.
"""
...
# BRENK ALERTS
# =============================================================================
# Source: Supporting Information of Brenk (2008)
# DOI: 10.1002/cmdc.200700139
# Copyright: ChemMedChem (Wiley-VCH, 2007)
# Total entries: 105 (including duplicate names with different SMARTS)
#
# Brenk alerts are structural groups associated with poor ADMET properties
# or mutagenicity. These patterns identify compounds that may have:
# - Poor bioavailability
# - Toxicity risks
# - Metabolic instability
# - Pan-assay interference

BRENK_ALERTS: dict[str, str] = {
    ...
    "chinone_1":                                "C1(=[O,N])C=CC(=[O,N])C=C1",
    "chinone_2":                                "C1(=[O,N])C(=[O,N])C=CC=C1",
    ...
    "cycloheptane_1":                           "[CR2]1[CR2][CR2][CR2][CR2][CR2][CR2]1",
    "cycloheptane_2":                           "[CR2]1[CR2][CR2]cc[CR2][CR2]1",
    "cyclooctane_1":                            "[CR2]1[CR2][CR2][CR2][CR2][CR2][CR2][CR2]1",
    "cyclooctane_2":                            "[CR2]1[CR2][CR2]cc[CR2][CR2][CR2]1",
    "diaminobenzene_1":                         "[cR2]1[cR2]c([N+0X3R0,nX3R0])c([N+0X3R0,nX3R0])[cR2][cR2]1",
    "diaminobenzene_2":                         "[cR2]1[cR2]c([N+0X3R0,nX3R0])[cR2]c([N+0X3R0,nX3R0])[cR2]1",
    "diaminobenzene_3":                         "[cR2]1[cR2]c([N+0X3R0,nX3R0])[cR2][cR2]c1([N+0X3R0,nX3R0])",
    "diazo group":                              "[N!R]=[N!R]",
    ...
    "halogenated ring_1":                       "c1cc([Cl,Br,I,F])cc([Cl,Br,I,F])c1[Cl,Br,I,F]",
    "halogenated ring_2":                       "c1ccc([Cl,Br,I,F])c([Cl,Br,I,F])c1[Cl,Br,I,F]",
...
#   "Michael acceptor_1":                       "C=!@CC=[O,S]",
    ...
    "Michael acceptor_5":                       "[$([CH]),$(CC)]#CC(=O)O[C,c]",
    "N oxide":                                  "[NX2,nX3][OX1]",
    ...
    "Polycyclic aromatic hydrocarbon_1":        "a1aa2a3a(a1)A=AA=A3=AA=A2",
    "Polycyclic aromatic hydrocarbon_2":        "a21aa3a(aa1aaaa2)aaaa3",
    "Polycyclic aromatic hydrocarbon_3":        "a31a(a2a(aa1)aaaa2)aaaa3",
    "polyene":                                  "[CR0]=[CR0][CR0]=[CR0]",
    "quaternary nitrogen_1":                    "[s,S,c,C,n,N,o,O]~[nX3+,NX3+](~[s,S,c,C,n,N])~[s,S,c,C,n,N]",
    "quaternary nitrogen_2":                    "[s,S,c,C,n,N,o,O]~[n+,N+](~[s,S,c,C,n,N,o,O])(~[s,S,c,C,n,N,o,O])~[s,S,c,C,n,N,o,O]",
    "quaternary nitrogen_3":                    "[*]=[N+]=[*]",
    ...
    "Sulfonic acid_1":                          "[C,c]S(=O)(=O)O[C,c]",
    "Sulfonic acid_2":                          "S(=O)(=O)[O-,OH]",
...
# PAINS ALERTS
# =============================================================================
# Source: Baell & Holloway (2010) J. Med. Chem. 53(7), 2719-2740
# DOI: 10.1021/jm901137j
# Pan-Assay Interference Compounds (PAINS) filter
#
# PAINS are compounds that show activity in many different assay types,
# often due to interference with the assay readout rather than true target
# binding. Three classes are defined:
#   a: 16 patterns  (high-frequency hitters)
#   b: 55 patterns  (moderate-frequency hitters)
#   c: 409 patterns (lower-frequency hitters)
# Total: 480 patterns
#
# Each entry maps the original filter name (with hit count from the paper)
# to its SMARTS pattern. The hit count in parentheses (e.g., 483) indicates
# how many times that substructure appeared across the assay panel.

PAINS_ALERTS: dict[str, dict[str, str]] = {
    # ---- Class a (16 patterns) ----
    "a": {
