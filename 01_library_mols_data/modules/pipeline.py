...
def stage_path(
    ...
    Returns:
        Path to the stage file in the appropriate .pipeline subdirectory.
    """
...
def checkpoint_path(stage_name: str) -> Path:
    ...
    Returns:
        Path like .pipeline/02_oxazolones/.cache/Oxazolones_checkpoint.json
    """
    base_dir = Path(_STAGE_REGISTRY[stage_name])
...
class CheckpointManager:
    ...
    Example:
        >>> manager = CheckpointManager("Oxazolones", Path(".pipeline/02_oxazolones"))
...
def _get_stage_dir(stage_name: str) -> Path:
    ...
    if base is not None:
        return Path(base)
    return Path(f"01_library_mols_data/.interim/{stage_name}")
